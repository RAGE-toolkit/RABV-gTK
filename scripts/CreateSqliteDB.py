...
class CreateSqliteDB:
    ...
    # NEW: auto-detect delimiter for manifest (tab/csv)
    @staticmethod
    ...
    def load_trees_from_dir(self, tree_dir: str) -> pd.DataFrame:
        ...
        tree_dir must contain:
          - meta_data.tsv (tree manifest) with columns:
              Chromosome, segment_number, tree_type, tree_name, tree_model
          - tree files referenced by tree_name

        Returns dataframe with manifest columns +:
          - tree_path
          - newick (full file content)
        """
    ...
    def create_db(self):
        ...
        df_host_lineage = pd.read_csv(join(self.host_lineage_file), sep="\t", dtype=str)
        df_host_children = pd.read_csv(join(self.host_children_file), sep="\t", dtype=str)
        df_fasta_sequences = self.load_fasta()

        # NEW: optional trees table (multi-tree)
        df_trees = None
        if self.tree_dir:
        ...
        # NOTE: These CREATE TABLE ... AS SELECT are redundant after to_sql(replace),
        # but keeping your style. Also fixed the host_lineage line.
        ...
        cursor.execute("""CREATE TABLE IF NOT EXISTS host_lineage AS SELECT * FROM host_lineage;""")  # FIXED
        cursor.execute("""CREATE TABLE IF NOT EXISTS host_children AS SELECT * FROM host_children;""")
...
if __name__ == "__main__":
    ...
    # NEW: replaces --ref_tree
    parser.add_argument(
        "-td",
        "--tree_dir",
        help="Directory containing tree files and a manifest meta_data.tsv (Chromosome, segment_number, tree_type, tree_name).",
        default=None,
    )
