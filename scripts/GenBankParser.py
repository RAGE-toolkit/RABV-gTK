...
'''
Normal mode:
	python scripts/GenBankParser.py  --exclusion_list generic/rabv/exclusion_list.txt --ref_list generic/rabv/ref_list.txt
Update mode:
	python scripts/GenBankParser.py  --exclusion_list generic/rabv/exclusion_list.txt --ref_list generic/rabv/ref_list.txt --update
'''
class GenBankParser:
	def __init__(
		self,
		...
		# keep user override (may be None)
		self.input_dir = input_dir

		# set input_dir default + out_root once
		self._init_update_mode_paths(redirect_output=True)

		# keep DB-related attribute around (but unused unless you re-enable it)
		...
		#self._init_update_mode_paths(redirect_output=False)
		#self._init_update_mode_paths()
		#self._existing_accessions = set()

	def _init_update_mode_paths(self, redirect_output=False):
		
		if not self.input_dir:
			if self.update_mode:
				self.input_dir = join(self.base_dir, "Update", "GenBank-XML")
			else:
				self.input_dir = join(self.base_dir, "GenBank-XML")

		if self.update_mode and redirect_output:
...
if __name__ == "__main__":
	...
	parser.add_argument(
		'--update',
		action='store_true',
		help='Update mode: read XMLs from <base_dir>/Update/GenBank-XML'
	)
